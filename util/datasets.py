...
class InstrcutDataSet(Data.Dataset):
    def __init__(self, args, split, model_path, max_words=512, max_image_feats=1):
        ...
        self.sqa_train_path = os.path.join(args.data_root, 'images', 'train')
        self.coco_train_path = os.path.join(args.data_root, 'cocoimages', 'train')
    ...
    def __getitem__(self, idx):
        ...
        if self.data[idx]['image'] is not None:
            ...
            if self.data[idx]['image_source'] == 'sqa':
                image = Image.open(os.path.join(self.sqa_train_path, self.qids[idx], 'image.png')).convert('RGB')
            else:
                image = Image.open(
                    os.path.join(self.coco_train_path, 'COCO_train2014_' + self.data[idx]['image'])).convert('RGB')
            image = self.transforms(image)
            indicator = 1
        else:
            image = torch.Tensor(torch.zeros(3, 224, 224).float())
